...
export class AsyncTestHelper {
  static async waitFor<T>(
    ...
    throw new Error(`Condition not met after ${retries} attempts. Last error: ${lastError?.message}`);
  }
...
export class TestDataGenerator {
  private static readonly LOREM_IPSUM = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.';
  
  static string(length: number = 10, charset: string = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'): string {
    let result = '';
...
export class EventTestHelper {
  ...
  static async waitForEventCount(
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Event count not reached. Expected: ${expectedCount}, Actual: ${count}`));
      }, timeout);
...
export class PerformanceTestHelper {
  ...
  static async measureMemoryUsage<T>(
    fn: () => Promise<T> | T
  ): Promise<{ result: T; memoryDiff: { rss: number; heapTotal: number; heapUsed: number; external: number } }> {
...
export class TestAssertionHelper {
  ...
  static assertAsyncComplete(promise: Promise<any>, timeout: number = 1000): Promise<void> {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
