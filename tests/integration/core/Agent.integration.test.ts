...
describe('Agent 集成测试', () => {
  ...
  describe('错误处理集成', () => {
    test('应该在组件初始化失败时正确回滚', async () => {
      ...
      jest.spyOn(failingAgent as any, 'initComponents').mockImplementationOnce(async () => {
        throw new Error('Component initialization failed');
      });
      
      await expect(failingAgent.init()).rejects.toThrow('Component initialization failed');
    ...
    test('应该在组件执行失败时正确处理', async () => {
      // 模拟 LLM 调用失败
      const llmManager = (agent as any).llmManager;
      jest.spyOn(llmManager, 'chat').mockRejectedValueOnce(new Error('LLM service unavailable'));
