...
describe('文件系统工具 E2E 测试', () => {
  ...
  describe('文件写入工具测试', () => {
    ...
    test('应该能够处理写入权限错误', async () => {
      ...
      const result = await session.runCommand(
        'tools', 
        ['run', 'file-write', '/root/test.txt', 'test content']
      );
  ...
  describe('错误处理测试', () => {
    test('应该正确处理无效路径', async () => {
      const result = await session.runCommand(
        'tools', 
        ['run', 'file-read', '/invalid/path/file.txt']
      );
    ...
    test('应该正确处理权限错误', async () => {
      const result = await session.runCommand(
        'tools', 
        ['run', 'file-delete', '/root/system-file']
      );
    ...
    test('应该正确处理磁盘空间不足', async () => {
      ...
      const result = await session.runCommand(
        'tools', 
        ['run', 'file-write', join(testDirPath, 'test.txt'), 'x'.repeat(1000000000)]
      );
