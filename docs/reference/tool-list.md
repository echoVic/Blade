...
工具只通过 Provider 的原生工具调用协议执行。若用户明确要求调用当前可用工具，
模型却只返回 `{"tool_calls":[...]}` 这样的 JSON 文本，Runtime 会发出内部纠正，
让模型改用原生调用，或在操作已完成时根据已有结果回答，不会解析文本后直接执行。
每次循环调用最多纠正两次，且不延长显式回合上限；预算耗尽时返回
`intent_fulfillment_failed`，而不是成功。此计数不承诺跨进程恢复。

若调用封装只涉及一个明确要求的可用工具，且本次循环尚无该工具的成功结果，纠正回合会指定该原生工具；模型仍只描述将要执行、或耗尽输出预算时明确失败。工具结果处理后或用户中途改向时解除约束；多工具封装不强制选择其中一个工具，文本参数始终不会直接执行。

该检查保守匹配纯 JSON 调用封装，不处理混合说明、代码围栏或未知格式；普通 JSON
示例、引用文本及结构化输出契约不走这条纠正路径。权限、技能工具限制和审批仍由
原有执行管线决定。内部纠正消息会持久化供模型续接，但不显示为用户输入。
