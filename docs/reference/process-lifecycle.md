...
- Provider physical stream 默认绕过进程内 admission，由真实 `429`、
  `retry-after` 与共享熔断器处理背压。只有显式配置
  `providerRequestConcurrency`、`providerGlobalConcurrency` 或
  `providerOwnerConcurrency` 时才创建 process-wide 有界公平调度器；未配置的层级和
  foreground/background/internal class 不施加隐藏 active 上限。启用后，pending
  count 与 retained-footprint bytes 仍保持有界，queued ticket 每 15 秒 heartbeat
  并响应统一 AbortSignal。permit 只覆盖一次真实 iterator；error/EOF/consumer
  return 后先释放，再进入 retry、circuit wait 或 fallback。`provider_admission`
  只暴露脱敏 resource 类型，不持久化 request bytes、domain/owner/Session identity。
...
- 自动压缩分为预测式 threshold compaction 与反应式 context-limit recovery。Provider
  ...
  循环。
- 预测式检查使用完整上下文占用，而不是上一请求的 prompt-only usage。最新 Provider
  usage 以 `max(totalTokens, promptTokens + completionTokens)` 作为保守基线；下一次请求
  前只追加估算该响应之后产生的 tool result、user/control message，以及 system prompt
  或 tool schema 的正向增长，不重复计算已经包含在 Provider total 中的 assistant
  response。模型或 schema 切换继续保留 Provider floor；usage 缺失、请求边界不匹配，
  或 compaction/rewind/恢复等历史重写后，改为对完整 system、tools 和 history 做本地
  估算。70% handoff 与 80% compaction 共用该投影，并都从扣除 output reserve 后的
  input budget 计算。
- compaction summary 使用独立的单层三次采样预算。内部 ChatService 禁用自动重试，
  由 CompactionService 统一分类：`408/409/429`、`5xx`、网络、超时、断流和空摘要
  可以指数退避重试；认证、权限、请求参数与 caller abort 立即停止。context overflow
  不会重放相同 payload，而是在同一三次总预算内按“移除可重读文件 → 丢弃最旧完整
...
- compaction 在重试 Provider 前必须先原子提交 exact replacement context。JSONL
  ...
  pre/post tokens，并记录 `preTokenSource`、`estimatedPendingTokens`、
  `sampleAttempts`、`inputReductions`、`messagesOmitted`、`filesOmitted`、
  `imagesOmitted`、`fallbackTargetTokens`、
...
- `compaction` LoopEvent 统一携带 `threshold|context_limit|turn_limit` reason、
  ...
  计数、计数来源、响应后估算增量、fallback token 目标与有界失败分类。TUI 与 Web 的
  context meter 都使用完整 `totalTokens / maxContextTokens`，不再让 TUI 只显示 prompt
  input。Web 在 reactive recovery 显示独立的 context-limit 状态；TUI、Headless
