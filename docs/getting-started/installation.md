...
TUI 启动时只用一小时内的有效本地版本缓存决定是否显示更新提示；缓存缺失或
过期时，网络查询在后台完成，不阻塞进入界面，也不在当前会话中途弹出更新提示。
查询结果供下次启动使用；需要立即检查时运行 `blade update`。版本查询的五秒
截止时间覆盖连接和正文读取。内置 Skills 随包提供，首次启动和工作区初始化
不再自动下载默认技能；需要其他技能时可通过 Web 设置显式安装。
