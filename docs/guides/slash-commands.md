...
侧边请求通过静态系统提示限定只回答当前侧问，历史对话仅供参考，不继续其中未完成或
已取消的主任务。历史用户文本在侧问的 Provider 请求中以 `main_conversation_reference`
显式引用，分隔符经过转义；角色、图片和元数据保留，父会话记录不变。当前问题不放入
历史引用，也不会把用户问题和历史正文提升为系统指令。

Web 端的侧边对话面板可在当前页面连续追问；这些追问只在面板打开期间作为临时上下文
传递，关闭面板即清除，不会写入主会话。选中主对话中的文字后，也可以直接将其作为
引用上下文打开侧边对话。未发送草稿也仅属于当前面板：关闭、新建或切换会话/工作区
后不保留。已取消或被替换请求的迟到失败不会恢复到新面板；当前请求正常失败或校验
未通过时仍保留草稿，便于修改后重试。侧问完成或报错时，若用户正在主输入框或其他
编辑器输入，不会自动抢走焦点；延迟聚焦期间主动切换焦点也会保留用户的选择。

Web 主输入框、侧边对话和批注编辑器在输入法组合期间将按键交给输入法处理：选词的
Enter 不发送消息或提交批注，Escape 不关闭面板，方向键不切换输入历史。选词结束后
再按 Enter 才提交；Shift+Enter 仍换行，Ctrl/Cmd+A 仍可全选。
