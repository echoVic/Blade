...
// 配置加密
export { ConfigEncryptor } from './config-encryptor';

// 命令执行安全
export { CommandExecutor } from './command-executor';

// 提示词安全
export { PromptSecurity } from './prompt-security';

// 错误处理
export { ErrorHandler } from './error-handler';

// HTTP 客户端安全
export { SecureHttpClient } from './secure-http-client';
...
// 安全装饰器
export function Secure(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
...
function validateResult(result: any): any {
  ...
  if (typeof result === 'string') {
    // 检查是否包含敏感信息
    const sanitized = ErrorHandler['sanitizeError'](result);
    if (sanitized !== result) {
