...
export class ChatRecordingService {
  ...
  // 列出所有录制
  public async listRecordings(options?: ListRecordingsOptions): Promise<ChatRecordingInfo[]> {
    try {
  ...
  public async getRecordingStats(): Promise<RecordingStats> {
    ...
    return {
      ...
      totalMessages,
      totalDuration,
      averageMessagesPerRecording: recordings.length > 0 ? totalMessages / recordings.length : 0,
      averageDurationPerRecording: recordings.length > 0 ? totalDuration / recordings.length : 0,
