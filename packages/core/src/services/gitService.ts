...
export class GitService {
  ...
  // 状态检查
  public async status(repoPath: string, options?: GitStatusOptions): Promise<GitStatus> {
    try {
  ...
  public async addRemote(
    ...
    try {
      const result = await this.executeGitCommand(['remote', 'add', name, url], { cwd: repoPath });
  ...
  public async removeRemote(
    ...
    try {
      const result = await this.executeGitCommand(['remote', 'remove', name], { cwd: repoPath });
  ...
  public async getCurrentBranch(repoPath: string): Promise<string> {
    try {
      const result = await this.executeGitCommand(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: repoPath });
      return result.stdout.trim();
    } catch (error) {
  ...
  public async getLastCommit(repoPath: string): Promise<GitCommit> {
    try {
      const result = await this.executeGitCommand(['log', '-1', '--pretty=format:%H|%an|%ae|%ad|%s'], { cwd: repoPath });
      const [hash, authorName, authorEmail, date, message] = result.stdout.trim().split('|');
...
type GitStatusType = 'added' | 'modified' | 'deleted' | 'renamed' | 'copied' | 'unmerged' | 'unknown';
