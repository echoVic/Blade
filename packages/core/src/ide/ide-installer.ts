...
export class IdeInstaller {
  private static readonly IDES: Record<string, IdeInstallationInfo> = {
    'vscode': {
      ...
      installerType: 'auto',
      executable: 'code',
      postInstallCommands: ['code --install-extension ms-vscode.vscode-typescript-next']
    },
    'webstorm': {
      name: 'WebStorm',
      downloadUrl: {
  ...
  public static async installIde(ideName: string, options: InstallOptions = {}): Promise<InstallResult> {
  ...
  private static async autoInstall(ideInfo: IdeInstallationInfo, options: InstallOptions): Promise<InstallResult> {
    ...
    try {
      // 下载安装包
      console.log('正在下载安装包...');
      const installerPath = await this.downloadInstaller(downloadUrl, tempDir, ideInfo.name);
  ...
  private static async manualInstall(ideInfo: IdeInstallationInfo, options: InstallOptions): Promise<InstallResult> {
    const platform = os.platform();
    const downloadUrl = ideInfo.downloadUrl[platform];
  ...
  private static async downloadInstaller(url: string, tempDir: string, ideName: string): Promise<string> {
  ...
  private static async runInstaller(installerPath: string, options: InstallOptions): Promise<void> {
    const platform = os.platform();
  ...
  private static async runWindowsInstaller(installerPath: string, options: InstallOptions): Promise<void> {
    // Windows安装命令
    const installCmd = `${installerPath} /S`; // 静默安装
    execSync(installCmd, { stdio: 'inherit' });
  }

  private static async runMacInstaller(installerPath: string, options: InstallOptions): Promise<void> {
    if (installerPath.endsWith('.dmg')) {
      ...
      execSync(`cp -R "/Volumes/${appName}/${appName}.app" /Applications/`, { stdio: 'inherit' });
  ...
  private static async runLinuxInstaller(installerPath: string, options: InstallOptions): Promise<void> {
    if (installerPath.endsWith('.tar.gz')) {
      // 解压到/usr/local
      execSync(`sudo tar -xzf "${installerPath}" -C /usr/local/`, { stdio: 'inherit' });
    } else {
      // 尝试直接执行
      execSync(`chmod +x "${installerPath}" && "${installerPath}"`, { stdio: 'inherit' });
    }
  }
  ...
  private static async findExecutable(executableName: string): Promise<string | null> {
    ...
    // 尝试在PATH中查找
    try {
      const whichResult = execSync(`which ${executableName}`, { encoding: 'utf-8' }).trim();
      if (whichResult) {
        return whichResult;
      }
    } catch {
  ...
  private static async getIdeVersion(executableName: string): Promise<string> {
    try {
      const versionOutput = execSync(`${executableName} --version`, { encoding: 'utf-8' });
