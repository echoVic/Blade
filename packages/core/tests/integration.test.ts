...
describe('Core 包集成测试', () => {
  ...
  it('应该支持多个配置层合并', () => {
    const result = createConfig({
      global: {
        auth: { apiKey: 'global-key', baseUrl: 'https://global.com', modelName: 'global-model', searchApiKey: '' }
      },
      user: {
        auth: { apiKey: 'user-key', baseUrl: 'https://user.com', modelName: 'user-model', searchApiKey: '' }
      }
    });
