  ...
  describe('createConfig', () => {
    ...
    it('应该合并多个配置层', () => {
      const layers: ConfigLayers = {
        global: {
          auth: { apiKey: 'global-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' },
          ui: { theme: 'dark', hideTips: false, hideBanner: false }
        },
        user: {
          auth: { apiKey: 'user-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' },
    ...
    it('应该按照优先级顺序合并配置', () => {
      const layers: ConfigLayers = {
        global: { auth: { apiKey: 'global-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' } },
        user: { auth: { apiKey: 'user-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' } },
        env: { auth: { apiKey: 'env-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' } }
      };
    ...
    it('应该确保所有必需字段都有默认值', () => {
      const layers: ConfigLayers = {
        global: {
          auth: { apiKey: 'custom-key', baseUrl: 'https://example.com', modelName: 'test-model', searchApiKey: '' }
          // 不提供其他字段
        }
      };
