...
// 生成快捷键的唯一键
const generateShortcutKey = (shortcut: Omit<KeyboardShortcut, 'description' | 'action'>): string => {
  const modifiers = [
    shortcut.ctrl ? 'Ctrl' : '',
    shortcut.alt ? 'Alt' : '',
...
export const useKeyboardShortcuts = (shortcuts: KeyboardShortcut[] = []) => {
  ...
  const handleInput = useCallback((input: string, key: { left: boolean; right: boolean; up: boolean; down: boolean; pageup: boolean; pagedown: boolean; return: boolean; escape: boolean; tab: boolean; backspace: boolean; delete: boolean; ctrl: boolean; alt: boolean; shift: boolean; meta: boolean }) => {
    // 如果输入是可打印字符，不是快捷键
    ...
    // 构建当前按键的快捷键
    const pressedKey = input.toUpperCase() || 
      (key.escape ? 'Escape' : '') ||
      (key.return ? 'Return' : '') ||
      (key.tab ? 'Tab' : '') ||
  ...
  // 移除快捷键
  const removeShortcut = useCallback((shortcut: Omit<KeyboardShortcut, 'description' | 'action'>) => {
    const key = generateShortcutKey(shortcut);
  ...
  const getShortcutsByCategory = useCallback((): ShortcutGroup[] => {
    ...
    const grouped = shortsuts.reduce((acc, shortcut) => {
...
export const useShortcutManager = () => {
  ...
  // 注销一组快捷键
  const unregisterShortcuts = useCallback((shortcuts: Omit<KeyboardShortcut, 'description' | 'action'>[]) => {
    shortcuts.forEach(removeShortcut);
  }, [removeShortcut]);

  // 检查快捷键冲突
  const checkConflict = useCallback((newShortcut: Omit<KeyboardShortcut, 'description' | 'action'>) => {
    const key = generateShortcutKey(newShortcut);
    const existingShortcuts = getAllShortcuts();
    return existingShortcuts.find(shortcut => 
      generateShortcutKey(shortcut) === key
    );
  }, [getAllShortcuts]);
  ...
  const validateShortcut = useCallback((shortcut: KeyboardShortcut): boolean => {
    ...
    // 检查是否至少有一个修饰键（除了单功能键）
    const isFunctionKey = ['Escape', 'Tab', 'Return', 'Backspace', 'Delete', 'Up', 'Down', 'Left', 'Right'].includes(shortcut.key);
